...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and len(context.args) > 0:
        ...
        if db.is_premium(user.id) or is_admin(user.id):
            ...
            keyboard = [
                [InlineKeyboardButton("🎖️ Get Premium", callback_data="show_plans")],
    ...
    if db.is_premium(user.id):
        ...
        keyboard = [
            [InlineKeyboardButton("💳 Pay with UPI", callback_data="show_plans")],
            [
...
HASEENA LINK (MAIN) - ₹299 to ₹650
HASEENA(2.0) - ₹149 to ₹320
SHEEP NEWS - ₹149 to ₹320
All-in-One - ₹699 to ₹899 (Best Value!)
"""
...
async def handle_payment_method(update: Update, context: ContextTypes.DEFAULT_TYPE, payment_type: str):
    ...
    keyboard = [
        [InlineKeyboardButton("Contact Admin", url=f"https://t.me/{config.ADMIN_USERNAME}?text={payment_label}%20Payment%20-%20TRX:%20{trx_id}%20-%20{channel}%20-%20{validity}%20-%20₹{amount}")],
        [InlineKeyboardButton("Back", callback_data="show_plans")],
    ]
...
async def check_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = "✅ Premium" if db.is_premium(user_id) else "❌ Free"
    expiry = db.get_premium_expiry(user_id) if user["is_premium"] else "N/A"
...
async def handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle new posts in the file channel - generate shareable link."""
...
application.add_handler(MessageHandler(
    filters.ChatType.CHANNEL & filters.Chat(config.CHANNEL_ID),
    handle_channel_post
))
