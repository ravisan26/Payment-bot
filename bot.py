    ...
    if context.args and len(context.args) > 0:
        ...
        if db.has_channel_access(user.id, channel_code) or is_admin(user.id):
            ...
            await update.message.reply_text(
                f"♻️ PREMIUM ♻️\n\n"
                f"💎 ᴛʜɪs ᴄᴏɴᴛᴇɴᴛ ɪs ᴘʀᴇᴍɪᴜᴍ ᴏɴʟʏ.\n\n"
                f"•  Pay and activate premium to get instant access.\n\n"
...
async def handle_plan_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if there's a photo message - if so we need to delete and send text
...
async def handle_payment_method(update: Update, context: ContextTypes.DEFAULT_TYPE, payment_type: str):
    ...
    keyboard = [
        [InlineKeyboardButton("Back", callback_data="show_plans")],
        [InlineKeyboardButton("Contact Admin", url=f"https://t.me/{config.ADMIN_USERNAME}?text={payment_label}%20Payment%20-%20{channel}%20-%20{validity}%20-%20Rs.{amount}")],
    ]
    ...
    await query.edit_message_text(
        ...
        f"Amount: ₹{amount}\n\n"
        f"✦ 𝗔𝗙𝗧𝗘𝗥 𝗣𝗔𝗬𝗠𝗘𝗡𝗧:\n"
        f"❐ Sᴇɴᴅ ᴀ ꜱᴄʀᴇᴇɴꜱʜᴏᴛ & ᴡᴀɪᴛ ᴀ ꜰᴇᴡ ᴍɪɴᴜᴛᴇꜱ ғᴏʀ ᴀᴄᴛɪᴠᴀᴛɪᴏɴ ✓\n\n"
...
async def admin_channel_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for plan_id, plan in plans.items():
        keyboard.append([
            InlineKeyboardButton(
                f"{plan['label']} : ₹{plan['price']} ({plan['days']} days)",
...
async def view_plans_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for plan_id, plan in sorted(plans.items()):
        line = f"`{plan_id}` - {plan['label']} - ₹{plan['price']} ({plan['days']} days)"
...
async def set_premium_image_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /setpremiumimage command - Admin only. Set the premium plans image."""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("You are not authorized.")
        return
    ...
    if update.message.reply_to_message and update.message.reply_to_message.photo:
        ...
        success = db.set_setting('premium_image_url', file_id)
        if success:
            config.PREMIUM_IMAGE_URL = file_id
            await update.message.reply_text(
                "**PREMIUM IMAGE SET**\n\n"
                "The image will now be shown when users view plans.",
                parse_mode="Markdown"
            )
        else:
        ...
        success = db.set_setting('premium_image_url', image_url)
        if success:
            config.PREMIUM_IMAGE_URL = image_url
            await update.message.reply_text(
                "**PREMIUM IMAGE SET**\n\n"
                "The image will now be shown when users view plans.",
                parse_mode="Markdown"
            ...
            await update.message.reply_text("Failed to save image URL.")
    else:
        await update.message.reply_text(
            "Usage:\n"
            "1. Reply to a photo with /setpremiumimage\n"
            "2. Or: /setpremiumimage <image_url>"
        )
...
application.add_handler(CommandHandler("setpremiumimage", set_premium_image_command))
